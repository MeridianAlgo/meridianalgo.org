...
type ProviderKey = 'google' | 'github' | 'email';

interface ProviderSwitchError extends Error {
...
const providerMethodMap: Record<string, ProviderKey> = {
  'google.com': 'google',
  'github.com': 'github',
  password: 'email'
};

const handleProviderSwitchError = async (error: any): Promise<Error> => {
  ...
  let providerMessage = 'the original sign-in method you used.';
  let requiredProvider: ProviderKey | undefined;

  for (const method of signInMethods) {
    const mapped = providerMethodMap[method];
    if (mapped) {
      requiredProvider = mapped;
      break;
    }
  }

  if (requiredProvider === 'google') {
    providerMessage = 'your Google account.';
  } else if (requiredProvider === 'github') {
    providerMessage = 'your GitHub account.';
  } else if (requiredProvider === 'email') {
    providerMessage = 'your email and password.';
  }

  const message = `An account with ${email} already exists. Please sign in using ${providerMessage}`;
  console.warn(message);

  const providerError: ProviderSwitchError = new Error(message);
  providerError.name = 'ProviderSwitchError';
  providerError.code = 'auth/provider-switch-required';
  providerError.requiredProvider = requiredProvider;
  providerError.providerMethods = signInMethods;
...
export const signInWithGoogle = async () => {
  try {
    ...
    const processedError = await handleProviderSwitchError(error);
    console.error("❌ Firebase: Error signing in with Google:", processedError);
...
export const signInWithGithub = async () => {
  try {
    ...
    const processedError = await handleProviderSwitchError(error);
    console.error("❌ Firebase: Error signing in with GitHub:", processedError);
    throw processedError;
  }
};
