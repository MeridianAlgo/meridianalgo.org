            ...
            <div className="flex flex-wrap justify-center gap-6">
              {HERO_BUTTONS.map((button) => (
                <Link
                  key={button.name}
                  to={button.to}
                  className="px-8 py-3 text-lg font-medium rounded-lg border-2 border-white text-white hover:bg-blue-600 hover:border-blue-600 transition-all duration-300 flex items-center space-x-2 group"
                >
                ...
                <div className="flex space-x-6">
                  {[...OPEN_SOURCE_PROJECTS, ...OPEN_SOURCE_PROJECTS].map((project, index) => (
                    <div 
                      ...
                      className={`carousel-card flex-none w-80 bg-gray-900/80 p-6 rounded-2xl border ${project.borderColor} ${project.hoverBorderClass} transition-colors duration-300 h-full flex flex-col`}
                    >
